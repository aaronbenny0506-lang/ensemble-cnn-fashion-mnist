...
> same four IDX files Keras would download are included in `data/` (sourced
...
|------------------|---------------------|----------------|
| Ensemble (5 CNNs)| 0.50                | 0.34           |
| Single CNN       | 0.40                | 0.48           |

Full numeric results (including config) are in
`output/results_summary.json` and `output/results_summary.txt`.

### Comparison

With only 40 training images across 10 classes, results are noisy and
sample-size-dependent — this run shows the ensemble outperforming the
single model on validation (+0.10) but underperforming on test (-0.14).
